...
if __name__ == "__main__":
    ...
    for rec in v:
        ...
        #NOTE: only support proband
        dmg_allele = 1
        if rec[v.samples[0]].GT == '1/1' or rec[v.samples[0]].GT == '1|1':
          dmg_allele += 1
        ...
        if retain:
          txes = []
          for tx, mim_inherited in dTxes.iteritems():
            if tx not in dmg_allele_txes:
              dmg_allele_txes[tx] = [mim_inherited,0]
            if mim_inherited == 'as_dominant':
              dmg_allele_txes[tx][0] = mim_inherited
            dmg_allele_txes[tx][1] += dmg_allele
    v.stream.close()
    
    v = vcf.VCFParser(options.infile)
    ostream = open(options.outfile, 'w')
    v.add_meta_info("CLASS_TAG", "1", "String", "1:confirmed_benign, 2:frequent, 3:rare, 4:novel, s:splice, i:intronic_coding, e:exonic, n:noncoding_exonic, c:previously_known_pathogenic_vairant, g:previously_known_pathogenic_gene, I:previously_known_pathogenic_variant_in_non_exonic, D:hetero in autosomal dominant|homozygous(compound hetero) in autosomal recessive")
    ...
    for rec in v:
      ...
      if retain:
        for tx, mim_inherited in dTxes.iteritems():
          if dmg_allele_txes[tx][1]<2:
            if dmg_allele_txes[tx][0] == 'as_dominant':
              rec.info.CLASS_TAG += 'D'
              break
          else:
            rec.info.CLASS_TAG += 'D'
            break
