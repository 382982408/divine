...
class Clnsnp(db.DB):
    ...
    def load_clnsnp(self):
        clnsnp = {'GAD': {}, 'NHGRI': {}}
        stmt = "SELECT snp, gene, phenotype, refdb from clnsnp where "\
                                                    "asnstatus = 'Y'"
        results = self.execute(stmt).fetchall()
        for row in results:
            snp, gene, phen, refdb = row[0], row[1], row[2], str(row[3])
        ...
        for refid, val in clnsnp.iteritems():
