...
class Effpred(db.DB):
    ...
    def _predict(self):
        ...
        if self.ref and self.alt:
            stmt = 'SELECT sift_score,sift_pred,pp2_hvar_score,pp2_hvar_pred \
        ...
        if len(results) > 0:
            self.sift_score = results[0][0]
            if self.sift_score is not None:
                self.sift_pred = self.sift_names[str(results[0][1])]
            self.pp2_score = results[0][2]
            if self.pp2_score is not None:
                self.pp2_pred = self.pp2_names[results[0][3].strip()]

    def get_domains(self, chrom, start_pos, end_pos):
        """Retrieves Interpro domains that overlaps with the given variant
        positions"""
        domains = []
        stmt = "SELECT domains FROM nsfp WHERE chr='%s' AND pos "\
    ...
    def get_cadd(self, chrom, pos, ref, alt):
        ...
        cadd_raw, cadd_phred, refaa,  altaa = (None, None, None, None)
        stmt = "SELECT cadd_raw, cadd_phred, refaa, altaa FROM nsfp WHERE chr='%s' \
                AND pos=%d AND ref='%s' AND alt='%s' limit 1" \
                % (chrom.strip(), pos, ref, alt)
        results = self.execute(stmt).fetchall()
        if len(results) > 0:
            cadd_raw, cadd_phred, refaa, altaa = results[0]
        return cadd_raw, cadd_phred, refaa, altaa
