...
SCHEMA = ["""create table clnsnp
               ( clnid integer not null,
                 snp text,
                 gene text not null,
                 phenotype text not null,
                 asnstatus text not null,
                 pubmedid integer not null,
                 refdb text not null,
                 refdbid text not null)
            """,
...
class ClnSNPDB(db.DB):
            ...
            if filename == 'all.txt':
                ...
                with anyopen.openfile(infile) as stream:
                    for rec in csv.reader(stream, delimiter='\t'):
                        if len(rec) > 1:
                            if rec[14] != ' - ':
                                if 'Date Added to Catalog' in rec:
                                    continue
                                self.nhgrirs[rec[21]] = [rec[14].upper(),\
                                                rec[1], rec[7].lower().strip()]
                                yield clnsnp._make((rec[21], rec[14], rec[7],\
                                            'Y', rec[1], 'NHGRI', rec[0]))
    ...
    def _insertclnsnp(self, entry, sid=None):
        ...
        if sid is None:
            sid = self._clnid + 1
        rginsert = 'insert into clnsnp values (' + ','.join(['?'] * 8) + ')'
